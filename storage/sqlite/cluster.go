...
func (db *DB) AddCluster(cluster *values.CouchbaseCluster) error {
	...
	_, err = db.sqlDB.Exec(`
		INSERT INTO clusters (uuid, name, nodes, buckets, info,  user, password, heartbeatIssue, cacert, lastUpdate)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`, cluster.UUID, cluster.Name, nodes, buckets, info, cluster.User,
		cluster.Password, cluster.HeartBeatIssue, cluster.CaCert, byteTime)
	if err != nil {
...
func (db *DB) GetClusters(sensitive bool) ([]*values.CouchbaseCluster, error) {
	clusters := make([]*values.CouchbaseCluster, 0)

	parameters := "uuid, name, nodes, buckets, info, heartbeatIssue, lastUpdate"
	if sensitive {
		parameters += ", user, password, cacert"
	}

	rows, err := db.sqlDB.Query("SELECT " + parameters + " FROM clusters ORDER BY uuid ASC;")
	if err != nil {
...
func (db *DB) GetCluster(uuid string, sensitive bool) (*values.CouchbaseCluster, error) {
	...
	row := db.sqlDB.QueryRow("SELECT "+parameters+" FROM clusters WHERE uuid = ?;", uuid)
...
func scanCluster(row scannable, sensitive bool) (*values.CouchbaseCluster, error) {
	...
	var nodes, byteTime, buckets, info []byte

	var err error
	if sensitive {
		err = row.Scan(&cluster.UUID, &cluster.Name, &nodes, &buckets, &info, &cluster.HeartBeatIssue, &byteTime,
			&cluster.User, &cluster.Password, &cluster.CaCert)
	} else {
		err = row.Scan(&cluster.UUID, &cluster.Name, &nodes, &buckets, &info, &cluster.HeartBeatIssue, &byteTime)
	}
