...
*Background*: Couchbase Server nodes regularly send heartbeat ticks to each other to collect stats from different Couchbase Server components.
If the Cluster Manager logs `dropped ticks` this means when it tried to process a tick, it found other ticks had not been processed yet.
This means that due to a scheduling issue, the Cluster Manager was not able to process the previous tick in time.
Dropped ticks are usually a sign of resource contention.

*Condition*: Can be triggered by either detecting over 10 dropped ticks on a node or by detecting over 4 instances of dropped ticks occurring on a node.

*Remediation*: Increase number of CPUs available to Couchbase Server.
...
*Background*: Having items in the disk write queue for too long could cause backoffs. 
