...
*Background*: Couchbase Server nodes regularly send heartbeat ticks between each other to make sure they are all active. If one of these ticks does not gets received it counts as a `dropped tick`. Dropped ticks are usually a sign of resource contention.
...
*Remediation*: Increase resources available to Couchbase Server node
