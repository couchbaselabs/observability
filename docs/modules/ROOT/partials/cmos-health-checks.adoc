...
[#CB90067]
=== Service Rollback to Zero (CB90067)

*Background*: A service has been forced to roll back to zero.
This may be undesirable because it means the service is attempting to resynchronize the entirety of the data set.

*Condition*: A rollback to zero is seen in the `memcached` logs.

*Remediation*: This is typically a symptom of another problem, you may ignore this unless you notice any other issues with your cluster.
If you do, then please Contact Couchbase Technical Support.
