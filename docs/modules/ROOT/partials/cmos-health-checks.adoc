...
*Background*: More then 50% of the memory is occupied by document keys and other metadata, this can happen when the document keys are large, or that there are many small documents.

*Condition*: Over 50% of memory is taken up by metadata.

*Remediation*: Increase memory allocation for bucket or change the https://docs.couchbase.com/server/current/rest-api/rest-bucket-create.html#evictionpolicy[evictionPolicy] of the bucket from `Value-only` (be aware this will have an adverse effect on performance).
