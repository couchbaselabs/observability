...
*Background*: Auto-Compaction can be set to run either at a specific time, or when bucket storage becomes too low.
Auto-Compaction requires some free space in order to run, if there is none available then compaction cannot run.

*Condition*: Auto-Compaction on bucket can fail if there is insufficient available storage.

*Remediation*: Allocate more storage to the nodes in bucket, {{ $labels.bucket }} and attempt to rerun Auto-Compaction.
You may also create a new cluster, with more storage, and use XDCR to transfer the files over to that, which should give you the space required for Auto-Compaction.
