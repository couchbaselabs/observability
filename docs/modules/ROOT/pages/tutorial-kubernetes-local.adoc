=  Configure CMOS for Kubernetes deployment on local using KIND
...
=== Create Kubernetes cluster in local using KIND

[IMPORTANT]
====
As we will be using KIND for running a Kubernetes cluster, it's important as first step to create a KIND cluster before we can do anything Kubernetes related.
====

The idea of this tutorial is to monitor a Couchbase cluster, running on https://kind.sigs.k8s.io/[KIND^] which would help us collecting Kubernetes related metrics, along with the standard Couchbase server metrics/health checks.

Once the cluster is created, set the name of the kubeconfig context to use.
...
==== Enable Ingress support for KIND

In order to manage external access to a Kubernetes services in a KIND cluster, we have to create a KIND cluster with ingress support. Please refer to the official KIND docs for adding https://kind.sigs.k8s.io/docs/user/ingress/[KIND Ingress support^]
...
For a production system it is likely an `Ingress` controller will already be deployed with appropriate rules. But for development in KINMD, we have to install it for the first time.

Follow this https://kubernetes.github.io/ingress-nginx/deploy/[Nginx Ingress Controller^] guideline to setup it.

[NOTE]
After installing the above controoler, check if the appropriate objects are created inside the `ingress-nginx` namespace.
...
[WARNING]
If developing on Apple M1(ARM based) machines, it is recommened to build your own image and use it in deployments. The OS/ARCH of the image `couchbase/observability-stack:latest` on Dockerhub is `linux/amd64`. For details, refer to https://github.com/couchbaselabs/observability#local-build-for-non-couchbase-employees[CMOS README^]. 
You can either upload the new image in your own repository or use https://kind.sigs.k8s.io/docs/user/quick-start/#loading-an-image-into-your-cluster[`kind load`^] to save it in the KIND cluster.
...
spec:
   ...
   template:
       ...
       spec:
           containers:
               - name: cmos
                 image: couchbase/observability-stack:latest # <.>
                 ports:
                     - name: http
                       containerPort: 8080
...
<.> The `couchbase/observability-stack:latest` image used can be found on https://hub.docker.com/r/couchbase/observability-stack/tags[Dockerhub^] here. For custom image build and use, see above.
...
spec:
   ports:
        - protocol: TCP
          # the service is exposed on the specified port internally within the cluster
          port: 80
          # the port on the pod/deployment that the request gets sent to.
          targetPort: 8080 # <.>
   selector:
       run: cmos
EOF
...
As we already have the Ingress controller installed and ready, the last step is to deploy the `Ingress` configuration as shown below :
...
If everything is deployed properly, you should be able to access http://localhost(You can modify yoiur Ingress rules to route different requests by their path to direct traffic to different services.)
