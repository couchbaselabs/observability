...
Couchbase Monitoring and Observability Stack (also known as CMOS) is a simple, out-of-the-box solution built using industry-standard tooling to observe the state of a running Couchbase Server cluster.
...
We also assume that you have Docker installed. If not, follow the https://docs.docker.com/engine/install/[official Docker installation instructions].

First, create a named Docker network to enable the components to resolve each other by their hostnames:
...
We plan for the image to be distributed on DockerHub, but currently this is not available. 
If you have a preview image given to you by Couchbase, use this. 

Run the Docker container, providing Prometheus and Alertmanager URLs to enable auto-discovery and configuration (after we have installed and configured them):
[source, console]
----
docker run -d --rm \
  -p 7196:7196 \ <.>
  ...
  --alertmanager-urls http://prometheus:9093
  --prometheus-label-selector job=couchbase-server <.>
...
<.> `prometheus` - the name (resolving to IP) of the Prometheus Docker container, on port `9090` (Prometheus), and port `9093` (Alertmanager).
<.> `job=couchbase-server` is the label we gave to the Couchbase Server Prometheus `scrape_config`.

Note that, when Prometheus auto-discovery is enabled, the Cluster Monitor will assume all your clusters are in Prometheus and stop monitoring any that are not.


== Install Prometheus

First, create a configuration file for Prometheus - by convention this is called `prometheus.yml`.
It will need to reference all your Couchbase Server nodes, and the Cluster Monitor (if this will be used). For example:
...
scrape_configs:
  ...
  - job_name: couchbase-cluster-monitor
      basic_auth:
        username: <Cluster Monitor Username>
        password: <Cluster Monitor Password>
...
Next, run Prometheus, mounting your configuration file inside the container.
...
If it starts successfully, you should be able to access its UI and see your cluster listed on http://localhost:9090/targets.
...
To take advantage of these, first download the rule definitions:
...
<.> `global` specifies the default settings for all alert receivers.
...
<.> `matchers` specifies which alerts will match this route - in this case, those with a severity of either `warning` or `critical`.
<.> `receiver` specifies which receiver matching alerts are sent to - in this case we are using `noop`.
<.> If `continue` is set, the alert will proceed through this route's sibling routes, otherwise it will stop here.
<.> `receivers` is where we define all receivers. In this case, we only define one, named `noop`, but you will likely define others.
...
[TIP]
====
When previously adding the rules file, it was necessary to stop and re-create the Prometheus container.
However, it is not necessary to restart it only modifying the configuration file.
...
----
docker run -d \
  -v $(pwd)/loki.yaml:/etc/loki/local-config.yml \
...
Just like Prometheus, Loki allows you to configure rules that send alerts if certain https://grafana.com/docs/loki/latest/logql/log_queries/[LogQL queries] match your logs.
The format of the rules file is almost identical to Prometheus', the only difference being that it uses https://grafana.com/docs/loki/latest/logql/[LogQL] instead of PromQL.
...
https://issues.couchbase.com/browse/CMOS-303

== Install Grafana

Unlike the other components, Grafana can be configured entirely through a web interface, and thus we do not need to prepare any configuration beforehand.
