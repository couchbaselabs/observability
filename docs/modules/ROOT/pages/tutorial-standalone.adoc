...
docker run -d \
  ...
  -v /tmp/prometheus/data:/prometheus \ # Persist data across restarts.
  -p 9090:9090 \
  --network cmos \
  --name prometheus \
  prom/prometheus
...
docker run -d \
  ...
  -v /tmp/alertmanager/data:/alertmanager \ # Persist data across restarts.
  -p 9093:9093 \
  --network cmos \
  --name alertmanager \
  prom/alertmanager
...
docker run -d \
  ...
  -v /tmp/loki:/tmp/loki \ # Persist data across restarts.
  -p 3100:3100 \
  --network cmos \
  --name loki \
  grafana/loki
...
<.> `job=couchbase-server` is the label we gave to the Couchbase Server Prometheus `scrape_config` earlier.


== Install Grafana

# This is missing downloading the default dashboards and installing them.
