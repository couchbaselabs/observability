...
First, download the rule defitions:
...
You should now be able to see the rules if you visit http://localhost:9090/rules (you may need to un-hide inactive rules):
...
Therefore, we should set the URL for the data source to `http://prometheus:9090`. We use basic auth, entering in the configured User and Password as `Administrator` and `password`.
...
Congratulations! You have now configured all of the components of CMOS individually.
