...
CMOS can be deployed to monitor the Couchbase clusters deployed via Couchbase Autonomous Operator.

== Deployment

CMOS is deployed on Kubernetes platform using a standard set of resources like Deployment, Services etc. Following sections describe how to deploy these standard objects.
...
Save the below content in a file called `prometheus-config.yaml`
...
<.> We want to be able to override in two specific ways: [[alert_rules]]
    ** Extend: provide Couchbase default rules and add custom ones.
    ** Replace: only provide custom rules.
<.> Alert Manager: The https://prometheus.io/docs/alerting/latest/alertmanager/[Alertmanager^] handles alerts sent by client applications such as the Prometheus server
<.> Scrape Config: Hold the configuration about fetching the metrics detail. [[Grafana_Section]]
...
<.> Get, List, Watch Permissions for resources: `services, endpoints, pods, secrets`
<.> Get, List, Watch Permissions for resources: `couchbaseclusters`

Execute the below command to create user roles in the cluster
...
To disable various services from being run, set a environment variable called `DISABLE_X` where `X` is an uppercase version of their entry point name (minus any .sh suffix). For example, to disable Loki you would set `DISABLE_LOKI`.
...
While setting up the prometheus config Grafana is also getting configured, refer <<Grafana_Section>> to see the configurations.
