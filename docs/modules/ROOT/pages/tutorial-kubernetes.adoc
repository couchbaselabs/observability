...
CMOS is a simple, out-of-the-box solution based on industry standard tooling to observe the state of your Couchbase cluster. xref:architecture.adoc[Architecture overview]
...
A prerequisite to configuring CMOS is to install kube-state-metrics. If not already installed you can install it using the following commands.
...
Save the below content in a file called `prometheus-config.yml`
...
----
kubectl create configmap prometheus-config-cmos --from-file="prometheus-config.yml"
----
