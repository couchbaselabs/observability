...
There are two ways of running it: on a subdomain (e.g. `cmos.example.com`), or on a subpath (e.g. `example.com/cmos`).
...
If you are running on a subpath, you will need to set the `CMOS_HTTP_PATH_PREFIX` environment variable to ensure all the services reflect this, for example `docker run -e "CMOS_HTTP_PATH_PREFIX=/cmos" ...`.
