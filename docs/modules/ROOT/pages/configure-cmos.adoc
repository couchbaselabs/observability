...
To mount files into a container, use the `docker -v <source>:<destination>` command, ideally as read-only.
...
- `CLUSTER_MONITOR_USER=${CLUSTER_MONITOR_USER:-admin}`: The username for the Couchbase cluster to monitor.
- `CLUSTER_MONITOR_PWD=${CLUSTER_MONITOR_PWD:-password}`: The password for the Couchbase cluster to monitor.
- `CLUSTER_MONITOR_ENDPOINT=${CLUSTER_MONITOR_ENDPOINT:-http://localhost:7196}`: The endpoint of the Couchbase cluster to monitor.
...
While this enables the addition of a new scrape target, it will not automatically add dashboards to Grafana. The dashboards will need to be added manually for the relevant scrape target.
...
Configuration file for the log aggregation system Loki is present in the YAML file located at  `/etc/loki/local-config.yaml` inside the container.
