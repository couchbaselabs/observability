...
* CMOS supports all Generally Available versions of Couchbase Server, however some features may only be supported on version 7.0 or later, or may need additional tools (such as the xref:health-agent.adoc[Health Agent]) to use all features.
