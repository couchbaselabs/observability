...
Before diving into this page, it is worth reviewing industry and standards bodies information on container licensing.
...
All licences are in the source repository and the microlith container in the link:https://github.com/couchbaselabs/observability/blob/main/microlith/licenses/[`/licenses`] directory.
...
The various external components are reused directly from the layers already published by the entities responsible (e.g. Grafana publish the Loki image layers): this container is not redistributing or repackaging them but using as-is.
...
These can all be seen in this excerpt from the link:https://github.com/couchbaselabs/observability/blob/main/microlith/Dockerfile[Dockerfile]:
