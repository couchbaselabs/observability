...
. **Docker**, to run the Cluster Monitor.
...
<.> `job=couchbase-server` is a label we will create later in this tutorial when configuring Prometheus' `scrape_config` to scrape our Couchbase Server nodes.

=== Install the Agent on your Couchbase Server nodes

[WARNING]
====
This agent is experimental. Please read the https://github.com/markspolakovs/yacpe/blob/master/README.md[documentation], specifically its disclaimer, before installation.
====
...
As there is currently no documentation on the planned deployment, this section cannot be written yet.
...
. **Cluster Monitor (JSON API)**, via the Web UI: click on _Add Data Source_ and select "JSON API". 
  - The URL should point to the Cluster Monitor, which listens on port `:7196`, with a sub-path of `/api/v1`.
...
. **Alertmanager** - this should already be configured as a Data Source. 
  ...
  - This should be named _Alertmanager API_, of type _JSON API_, and the URL should point to your Alertmanager instance. 
...
And add another `scrape_config` job for the previously-installed Cluster Monitor:
...
Restart Prometheus once more, and navigate to the Web UI. You should be able to see the new rules listed on the `/rules` page.
