...
See our xref:tutorial-standalone.adoc#_install_prometheus[Install Prometheus] documentation for an example of what this file should look like with an appropriate Couchbase Server `scrape_config`.
...
See xref:tutorial-standalone.adoc#_install_cluster_monitor[] on how to run the Cluster Monitor and configure it to discover your Prometheus and Alertmanager instances.
...
If you are using https://grafana.com/docs/grafana/latest/administration/provisioning/[provisioning], you will need to update the `providers.options.path` argument in your provisioning configuration file (typically named `grafana.yml`).
For example:
...
If you are utilising Grafana provisioning for dashboards, then your `grafana.yml` may look something like this:
[source, console]  
----
      ...
   - name: JSON API
...
Follow the xref:tutorial-standalone.adoc#_configuring_grafana_data_sources[] instructions for a general guide on how to do this.
