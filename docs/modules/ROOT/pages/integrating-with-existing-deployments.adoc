...
If you are not using the xref:deployment-microlith.adoc["microlith" image], you can deploy the Cluster Monitor stand-alone using Docker:
...
docker run -d --rm \
  ...
  couchbase/cluster-monitor:latest
----
...
<.> `targets` should specify the addresses of all the nodes in your cluster.
    ...
    If you are using the xref:health-agent.adoc[Health Agent], this will be `9092`.
