...
Now we have the required plugins and dashboards installed, we need to configure the required datasources:

. **Prometheus** - if you're running Grafana, you likely already have a Prometheus instance configured as a datasource. 
If not, the Standalone tutorial has instructions on xref:tutorial-standalone.adoc#_configuring_grafana_datasources[adding a Prometheus datasource].
. **Cluster Monitor (JSON API)** - click on _Add Datasource_ and select "JSON API". The URL should point to the Cluster Monitor, which listens on port `:7196`, with a subpath of `/api/v1`. 
