...
func MergeEventLogs(cred *values.Credentials) error {
	linePointers := []*logFile{}

	eventLog, err := os.OpenFile("events_"+cred.NodeName+".log", os.O_CREATE|os.O_WRONLY|os.O_APPEND, os.ModePerm)
	if err != nil {
		return err
	}
	...
	for _, log := range parsers.ParserFunctions {
		file, err := os.Open(log.Name + "_events.log")
		if err != nil {
		...
		for i, file := range linePointers {
			if name == file.Name {
				...
				if err == io.EOF {
					...
					zap.S().Warnw("Failed to merge line into events log", "log", file.Name, "err", err)
					break
				}
