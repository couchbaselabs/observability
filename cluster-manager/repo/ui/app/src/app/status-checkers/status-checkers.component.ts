...
import { Checkers, ClusterStatusResults, StatusResults } from '../types';
...
export class StatusCheckersComponent implements OnInit {
  ...
  getClusterIdentifier(): string {
    return this.name || this.clusterUUID;
  }

  getClusterStatusResults(): StatusResults[] {
    return (
      this.cluster?.status_results.filter((result: StatusResults) => {
        return result.cluster.length > 0 && !result.node && !result.log_file && !result.bucket;
      }) || []
    );
  }
  ...
  getStatusFilters(): string[] {
    let queryStatusFilter = this.route.snapshot.queryParamMap.get('statusFilter');
    try {
      if (queryStatusFilter) {
        let statusFilter: string[] = JSON.parse(queryStatusFilter);
        return statusFilter;
      }
    } catch {
  ...
  refreshCluster(uuid: string) {
    this.rest.refreshCluster(uuid).subscribe(
      () => {
        this.alertService.success('Cluster refresh triggererd. Rerunning checkers, this may take a couple of minutes');
      },
      (err) => {
        this.alertService.error(getAPIError(err));
      }
