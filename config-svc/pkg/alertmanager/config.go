...
func mergeYamlMaps(result *yaml.Node, sources ...*yaml.Node) error {
	slack_api_url_string := "slack_api_url"
	slack_api_url_file_string := "slack_api_url_file"

	slack_api_url_node := &yaml.Node{
	...
	for srcIdx, source := range sources {
		...
		for i := 0; i < len(source.Content); i = i + 2 {
			...
			for j := 0; j < len(result.Content); j = j + 2 {
				...
				if srcKey.Value == slack_api_url_string && resKey.Value == slack_api_url_file_string {
					slack_api_url_node.HeadComment = result.Content[j].HeadComment
					result.Content[j] = slack_api_url_node
					result.Content[j+1] = source.Content[i+1]
					found = true
				}
			}
