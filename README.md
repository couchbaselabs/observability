...
1. Ensure you have a Couchbase cluster running and accessible (whether using [Vagrant](https://github.com/couchbaselabs/vagrants), [Docker](https://docs.couchbase.com/cloud-native-database/containers/docker-basic-install.html), or [Kubernetes](https://docs.couchbase.com/operator/current/overview.html#getting-started))
2. Clone the couchbaselabs/observability repo: `git clone git@github.com:couchbaselabs/observability.git`
3. If you are a Couchbase employee, set up your SSH agent: `eval $(ssh-agent) && ssh-add`
4. Build the container: `make container` if you are a Couchbase employee or `make container-oss` otherwise
