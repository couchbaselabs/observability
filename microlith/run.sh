...
export ALERTMANAGER_URL_SUBPATH=${ALERTMANAGER_URL_SUBPATH-$CMOS_HTTP_PATH_PREFIX/alertmanager/}

export LOKI_CONFIG_FILE=${LOKI_CONFIG_FILE:-/etc/loki/config.yml}

export JAEGER_URL_SUBPATH=${JAEGER_URL_SUBPATH:-$CMOS_HTTP_PATH_PREFIX/jaeger}
...
if [[ -z "${DISABLE_ALERTMANAGER+}" ]]; then
    export CB_MULTI_ALERTMANAGER_URLS=${CB_MULTI_ALERTMANAGER_URLS:-"http://localhost:9093$CMOS_HTTP_PATH_PREFIX/alertmanager"}
fi
...
export CMOS_CFG_HTTP_PATH_PREFIX=${CMOS_CFG_HTTP_PATH_PREFIX:-$CMOS_HTTP_PATH_PREFIX/config}
