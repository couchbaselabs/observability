...
async function configureCluster(config) 
{
  ...
  async function addCluster(config) {
    ...
    const port = config.useTLS ? 18091 : 8091;
    
    // Prepare the request data
    const postData = JSON.stringify({
      host: `${config.hostname}:${port}`,
...
app.post('/api/configureCluster', async (req, res) => {
...
app.post('/api/loadAllClusters', async (req, res) => {
  try {
    if (fs.existsSync(configFilePath)) {
      ...
      const results = await Promise.all(clusters.map(configureCluster));
