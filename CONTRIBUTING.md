...
These tests are also run automatically using GitHub Actions. However, since setting up VMs and a Kubernetes cluster can be slow, we _only_ run the containers smoke suite on pull requests. We run the full suite of tests every night, or manually upon request by a maintainer.
